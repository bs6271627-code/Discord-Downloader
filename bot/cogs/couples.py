...
_GIF_BASE = "https://api.waifu.pics/sfw/{}"


async def _fetch_gif(session: aiohttp.ClientSession, action: str) -> str | None:
    try:
        async with session.get(_GIF_BASE.format(action), timeout=aiohttp.ClientTimeout(total=5)) as resp:
            if resp.status == 200:
...
class Couples(commands.Cog):
    ...
    async def marry(self, ctx: commands.Context, member: discord.Member) -> None:
        ...
        if view.result == "accepted":
            ...
            gif = await _fetch_gif(session, "hug")
    ...
    async def _gif_cmd(
        ...
        await ctx.defer()
        session = await self._get_session()
        gif = await _fetch_gif(session, gif_type)
